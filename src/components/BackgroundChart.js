...
const BackgroundChart = () => {
  ...
  const initialId = Object.keys(localizedDataMap)[0]; // Get first key from textMap
  const [selectedInfo, setSelectedInfo] = useState(localizedDataMap[initialId]);

  const navigateText = (direction) => {
    const keys = Object.keys(localizedDataMap);
    const currentIndex = keys.indexOf(Object.keys(localizedDataMap).find(key => localizedDataMap[key] === selectedInfo));
    let newIndex = currentIndex + direction;
  
    if (newIndex < 0) newIndex = keys.length - 1; // Loop to last item
    if (newIndex >= keys.length) newIndex = 0; // Loop to first item
  
    setSelectedInfo(localizedDataMap[keys[newIndex]]);
  };

  useEffect(() => {
    const initialId = Object.keys(localizedDataMap)[0];
    setSelectedInfo(localizedDataMap[initialId]);
  }, [localizedDataMap]);
        ...
        if (!isTouchDevice()) {
          svg.selectAll("g")
            .filter(function () {
              ...
              return Object.keys(localizedDataMap).includes(id);
            })
            ...
            .on("click", function () {
              const id = d3.select(this).attr("id");
              const info = localizedDataMap[id] || { title: "Unbekannt", text: "Keine Beschreibung verfügbar.", color: "#ccc" };
